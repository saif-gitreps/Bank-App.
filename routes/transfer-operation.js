...
router.post("/transfer/:id/accept", async (request, response) => {
   ...
   await db.query("insert into message_box(id, message) values(?,?)", [
      transferData[0][0].sender,
      `Transaction amount ${transferData[0][0].amount}AED was successfully sent to reciever account${transferData[0][0].receiver}`,
   ]);
   await db.query("insert into message_box(id, message) values(?,?)", [
      transferData[0][0].receiver,
      `Transaction amount ${transferData[0][0].amount}AED was successfully deposited in your account from account${transferData[0][0].sender}`,
   ]);
...
router.post("/transfer/:id/reject", async (request, response) => {
   ...
   await db.query("insert into message_box(id, message) values(?,?)", [
      transferData[0][0].sender,
      `Apologies. Transaction amount of${transferData[0][0].amount}AED to account ${transferData[0][0].receiver} was unsuccessful`,
   ]);
   response.redirect(`/admin/${transferData[0][0].admin_id}/transfer`);
});
